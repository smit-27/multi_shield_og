...
router.post('/:id/approve', (req, res) => {
  ...
  runSql("UPDATE incidents SET status='approved',resolution='Approved by admin',resolved_by=?,resolved_at=datetime('now') WHERE id=?", [approved_by, req.params.id]);
  runSql("UPDATE approval_requests SET status='approved',admin_response='Approved by admin',resolved_by=?,resolved_at=datetime('now') WHERE incident_id=?", [approved_by, req.params.id]);
  runSql("INSERT INTO audit_log (event_type,details,performed_by) VALUES ('incident_approved',?,?)", [JSON.stringify({ incident_id: req.params.id }), approved_by]);
